...
//declerad the version that we are going to use

//CREATING OUR SMART CONTRACT //
//1-This is the smart contract that handles all of the business logic 
//for buying and selling the items on blockchain.
//2-Writes and reads from blockchain

contract SurveyReward {
    mapping(uint256 => Survey) public surveys;
    //A hash table. A key and a value.
    //Key => Survey id, value => whole survey struct
    uint256 public surveyCount = 0;
    //We can use how many surveys are inside the mapping.
    //We increment this everytime we add a new survey.
    string public name;
    //State variable which value is stored on the blockchain.
    //Since we want to use the variable outside of the contract we used "public"

    struct Survey {
    //Create a way to add surveys to the SurveyReward
    //in order to do that we use struct to create our own data structure.
        string title;
    ...
    //Constructor--> A function inside the solidity that runs only one time
    constructor() public {
        name = "Survey Rewarding System by BNB";
    }
    ...
    public payable {
        //Altı çizgili olanlar local variable
        require(bytes(_title).length != 0, "Survey title can not be empty!");
        require(bytes(_description).length != 0, "Survey description can not be empty!");
        ...
        //First we made sure that the parameters is correct

        //Create Survey
        surveys[surveyCount] = Survey(_title, msg.sender, 0, true,_reward,msg.value,_description);
        //we increment the Survey parameters by one. for the owner we use msg.sender 
        //and since we didn't fill the survey it's false at the beginning.
    ...
    // REQUIRED: BOTH SURVEY AND QUESTION SHOULD EXIST
